...
export class ImageZoom implements OnInit, OnDestroy, OnChanges {
    ...
    private isImageRotated: boolean = false;

    private _widthRatio: number = 3;
    private _heightRatio: number = 3;
    ...
    private imageChanged() {
        ...
        this._zoomedImage.onload = () => {
            ...
            this.zoomLevel = (this._maxZoomLevel < 2.5) ? (this._maxZoomLevel / 1.5) : 1.5;
    ...
    private setImageStateProperties() {
        switch (this.imageState) {
            case 'rotate-clockwise01':
                this.windowPosition = 1;
                this.isImageRotated = false;
                break;
            case 'rotate-clockwise11':
            case 'rotate-anti-clockwise31':
                this.windowPosition = 15;
                this.isImageRotated = true;
                break;
            case 'rotate-clockwise31':
            case 'rotate-anti-clockwise11':
                this.windowPosition = 31;
                this.isImageRotated = true;
                break;
            case 'rotate-clockwise21':
            case 'rotate-anti-clockwise21':
                this.windowPosition = 21;
                this.isImageRotated = false;
                break;
        }
        this.imageZoomContainer.setImageState(this.imageState);
        this.imageZoomLens.setImageState(this.imageState);
    }
    ...
    private calculateBoundaries(clientX: number, clientY: number) {
        ...
        if (this.isImageRotated) {
            ...
            clientY = temp;
             ...
             if (this.imageState == 'rotate-clockwise11' || this.imageState == 'rotate-anti-clockwise31') {
                clientX = clientX - this._elementOffsetY + this._elementOffsetX;
                clientY = (this.img.offsetParent.clientHeight + this._containerOffsetY + this.lensHeight / 2) - (clientY - this._elementOffsetX + this._elementOffsetY);

            } else if (this.imageState == 'rotate-clockwise31' || this.imageState == 'rotate-anti-clockwise11') {
               clientX = this.img.offsetParent.clientWidth + this._containerOffsetX + this.lensWidth / 2 - (clientX - this._elementOffsetY + this._elementOffsetX);
               clientY = clientY - this._elementOffsetX + this._elementOffsetY; 
            }


        } else if (this.imageState == 'rotate-clockwise21' || this.imageState == 'rotate-anti-clockwise21') {
            clientX = this.img.offsetParent.clientWidth + this._containerOffsetX + this.lensWidth / 2 - clientX;
            clientY = this.img.offsetParent.clientHeight + this._containerOffsetY + this.lensHeight / 2 - clientY;
        }

        xPos = clientX - this._elementOffsetX;
            ...
            yPos = clientY - this._elementOffsetY;
    ...
    ngOnChanges(changeRecord: SimpleChanges) {
        for (let prop in changeRecord) {
            if (!changeRecord[prop].isFirstChange()) {
                if (prop === 'imageZoom' || prop === 'imageState') { // Image changed
                    this.imageChanged();
